...
TOKEN = "5537243600:AAEOiIAfpk89B_0wawqFOrJ0DffYx48Ybr8"
OWM_KEY = "ed0b2d87cc61572f8ae096b3700e8d28"
...
def parseConfig():
    ...
    URL = "https://api.telegram.org/bot{}/".format("5537243600:AAEOiIAfpk89B_0wawqFOrJ0DffYx48Ybr8")
    URL_OWM = "http://api.openweathermap.org/data/2.5/weather?appid={}&units=metric".format("ed0b2d87cc61572f8ae096b3700e8d28")
    POLLING_TIMEOUT
...
def getWeather(place):
    if isinstance(place, dict):     # coordinates provided
        ...
        js = makeRequest(url)
        logger.debug(js)
        return u"%s \N{DEGREE SIGN}C, %s in %s \nHumidity:%s%% \nWind Speed:%skm/h" % (getTemp(js), getDesc(js), getCity(js), getHumidity(js),getWind(js))
...
def handleUpdates(updates):
    for update in getResult(updates):
        ...
        if text == "/weather":
            ...
            sendMessage("Please input location", chatId)
        elif text == "/start":
            sendMessage("Welcome to Misimu! Send me the address of any location and I will send you the current weather there.",chatId)
            sendMessage("Try /weather", chatId)
        elif text.startswith("/"):
            logger.warning("Invalid command %s" % text)    
            continue
        elif (text) and (chatId in chats) and (chats[chatId] == "weatherReq"):
            logger.info("Weather requested for %s" % text)
            # Send weather to chat id and clear state
            sendMessage(getWeather(text), chatId)
            del chats[chatId]
        else:
            keyboard = buildKeyboard(["/start"])
            sendMessage("Try /start", chatId, keyboard)
